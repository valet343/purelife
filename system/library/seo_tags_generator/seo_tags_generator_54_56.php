...
class SeoTagsGenerator
{
    ...
    public function parse($string, $var_values)
    {
        ...
        $string = preg_replace(array("/\\s+/", "/\\s\\./"), array(" ", "."), $string);
        return $string;
    }
    private function parseCondition($code_initial, $if_condition, $var_values)
    {
        ...
        preg_match("|" . str_replace(array("[", "]", "(", ")"), array("\\[", "\\]", "\\(", "\\)"), $if_condition["if_construction"]) . "(.*?)<else>|s", $code_initial, $match_if);
        if (count($match_if) < 1) {
            preg_match("|" . str_replace(array("[", "]", "(", ")"), array("\\[", "\\]", "\\(", "\\)"), $if_condition["if_construction"]) . "(.*?)</endif>|s", $code_initial, $match_if);
        }
        if (!isset($match_if[1])) {
            return false;
        }
    ...
    private function getVarValue($var, $var_values)
    {
        ...
        $var_key = trim(str_replace(array("[", "]"), array("", ""), $var));
        ...
        if (0 < count($matches_foo)) {
            foreach ($matches_foo as $key => $item) {
                if (!in_array($item[0], $category_keys_exist)) {
                    ...
                    $categories_keys[$key] = str_replace(array("[", "]"), array("", ""), $item[0]);
