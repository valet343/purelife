...
class StdE {
	...
	public function fieldExtensionMonolithic($key, $default_value) {
		...
		if (false === strpos($key2, 'module_' . $this->extension_code . '_')) {
			$key2 = 'module_' . $this->extension_code . '_' . $key2;
		}
	...
	public function breadcrumbsModuleWithBlocks() {
		...
		if (!isset($this->request->get['module_id'])) {
			$breadcrumbs[] = array(
				...
				'href'=>$this->url->link('extension/module/' . $this->extension_code, 'user_token=' . $this->session->data['user_token'], true)
			);
		} else {
			$breadcrumbs[] = array(
				'text'=>$this->language->get('heading_title'),
				'href'=>$this->url->link('extension/module/' . $this->extension_code, 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}
	...
	public function link($target) {
		...
		if ('index' == $target) {
			return $this->url->link('extension/module/' . $this->extension_code, 'user_token=' . $this->session->data['user_token'], true);
			...
			return $this->url->link('extension/module/' . $this->extension_code . '/' . $target, 'user_token=' . $this->session->data['user_token'], true);
		}
