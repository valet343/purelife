...
class StdE {
	...
	public function breadcrumbsModuleWithBlocks() {
		...
		if (!isset($this->request->get['module_id'])) {
			$breadcrumbs[] = array(
				...
				'href'=>$this->url->link('module/' . $this->extension_code, 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$breadcrumbs[] = array(
				'text'=>$this->language->get('heading_title'),
				'href'=>$this->url->link('module/' . $this->extension_code, 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
	...
	public function link($target) {
		...
		if ('index' == $target) {
			return $this->url->link('module/' . $this->extension_code, 'token=' . $this->session->data['token'], 'SSL');
		} elseif ('action' == $target) {
			return $this->url->link('module/' . $this->extension_code, 'token=' . $this->session->data['token'], 'SSL');
		} elseif ('cancel' == $target) {
			return $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');
		} elseif (false !== strpos($target, 'part')) {
			return $this->url->link('module/' . $this->extension_code . '/' . $target, 'token=' . $this->session->data['token'], 'SSL');
		}
	...
	public function languages($languages) {
		foreach ($languages as $key=> $language) {
			$languages[$key]['src'] = 'view/image/flags/' . $language['image'] . '.png';
		}
