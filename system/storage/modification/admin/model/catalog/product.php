...
class ModelCatalogProduct extends Model {
    ...
    // SEO Tags Generator . end
      ...
      if (!empty($data['gkd_extra_desc_fields']) && defined('GKD_UNIV_IMPORT')) {
		...
		if (isset($data['product_sticker'])) {
			foreach ($data['product_sticker'] as $product_sticker) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_sticker SET product_id = '" . (int)$product_id . "', sticker = '" . $this->db->escape($product_sticker['sticker']) . "', sort_order = '" . (int)$product_sticker['sort_order'] . "'");
			}
		}
    ...
    // OCFilter end
      ...
      if (!empty($data['gkd_extra_desc_fields']) && defined('GKD_UNIV_IMPORT')) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_sticker WHERE product_id = '" . (int)$product_id . "'");
	...
	private function delete_image($file_path) {
		if (!$this->user->hasPermission('modify', 'common/filemanager')) return;		
		$image_to_delete = rtrim(DIR_IMAGE  . $file_path);
		if (!file_exists($image_to_delete)) return; 		

		if (is_file($image_to_delete)) {
			unlink($image_to_delete);
		}
	}

	private function CountAllProductsWithImage($product_id,$image) {
		$query = $this->db->query("SELECT COUNT(*) + (SELECT COUNT(*) FROM `" . DB_PREFIX . "product_image` WHERE `image` = '" . $this->db->escape($image) . "' AND `product_id`<> '" . (int)$product_id . "') as cnt FROM `" . DB_PREFIX . "product` WHERE `image` = '" . $this->db->escape($image) . "' AND `product_id`<> '" . (int)$product_id . "'");
	...
	public function deleteProductWithImage($product_id) {		
		...
		if($query->row){				
			...
			$results[] = array('image' => $query->row['image']);
			foreach ($results as $result) {
				if (trim($result['image']) != '') {
					$count = $this->CountAllProductsWithImage($product_id,$result['image']);
					if ($count == 0) {
    ...
    if (!empty($data['filter_import_batch'])) {
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			...
			if($data['config_sort_new_product'] == 1) {
				$sql .= " ORDER BY p.product_id"; // customized
			} else {
