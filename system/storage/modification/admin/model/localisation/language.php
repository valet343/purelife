...
class ModelLocalisationLanguage extends Model {
	public function addLanguage($data) {
		...
		// For Module Default Formulas
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "setting WHERE code = 'module_seo_tags_generator'");

		foreach ($query->rows as $row) {
			if ($row['serialized'] == 1) {
				$row_with_lang_data = json_decode($row['value'], true);
				$row_with_lang_data[$language_id] = $row_with_lang_data[$this->config->get('config_language_id')];
				$row_json = json_encode($row_with_lang_data);
				
				$this->db->query("UPDATE " . DB_PREFIX . "setting SET value = '" . $this->db->escape($row_json) . "' WHERE `key` = '" . $this->db->escape($row['key']) . "'");
			}
		} 
		
		// For Specific Categories
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "seo_tags_generator WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");

		foreach ($query->rows as $row) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_tags_generator SET category_id = '" . (int)$row['category_id'] . "', language_id = '" . (int)$language_id . "', `key` = '" . $this->db->escape($row['key']) . "', value = '" . $this->db->escape($row['value']) . "'");
		}
