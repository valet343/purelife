...
class ControllerCatalogProduct extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $product_id) {
				...
				if($data['config_delete_photos_with_product'] == 1) {

				$this->model_catalog_product->deleteProductWithImage($product_id);
				} else {
      ...
      $data['filter_import_batch'] = $filter_import_batch;
		...
		*/
		...
		if (isset($this->request->get['sort'])) {
			...
			if($data['config_sort_new_product'] == 1) {
    ...
    if (isset($this->request->get['filter_import_batch'])) {
      $url .= '&filter_import_batch=' . urlencode(html_entity_decode($this->request->get['filter_import_batch'], ENT_QUOTES, 'UTF-8'));
		...
		$stg_product_data['minimum']			 = $data['minimum'];
		$stg_product_data['tax_class_id']	 = $data['tax_class_id'];
		$stg_product_data['quantity']			 = $data['quantity'];
		$stg_product_data['model']				 = $data['model'];
		$stg_product_data['sku']					 = $data['sku'];
		$stg_product_data['upc']					 = $data['upc'];
		$stg_product_data['ean']					 = $data['ean'];
		$stg_product_data['jan']					 = $data['jan'];
		$stg_product_data['isbn']					 = $data['isbn'];
		$stg_product_data['mpn']					 = $data['mpn'];

		//$stg_product_data['manufacturer']	 = $data['manufacturer']; // A-M!
		$stg_product_data['manufacturer_id'] = $data['manufacturer_id']; // A-M!
		$stg_product_data['model_synonym']	 = isset($data['model_synonym']) ? $data['model_synonym'] : false;
		$stg_product_data['price']					 = $data['price'];
		$stg_product_data['special']				 = $this->model_extension_module_seo_tags_generator->getSpecial($this->request->get['product_id']);
		$stg_product_data['rating']					 = $this->model_extension_module_seo_tags_generator->getRating($this->request->get['product_id']);
		$stg_product_data['reviews']				 = $this->model_extension_module_seo_tags_generator->getReviews($this->request->get['product_id']);
