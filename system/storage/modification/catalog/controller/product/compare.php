...
$this->load->model('extension/module/oc_watermark');
		...
		foreach ($this->session->data['compare'] as $key => $product_id) {
			...
			if ($product_info) {
				if ($product_info['image']) {
					$image = $this->model_extension_module_oc_watermark->resize('product', $product_info['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_compare_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_compare_height'));
				} else {
					$image = false;
				}
	...
	public function add() {
		...
		if ($product_info) {
			...
			$json['link_checkout'] = $this->url->link('product/compare', '', true);
			$json['text_checkout'] = sprintf($this->language->get('button_compare'));
			...
			$json['total_pro'] = sprintf($this->language->get('text_compare_pro'), (isset($this->session->data['compare']) ? count($this->session->data['compare']) : 0));
		}
