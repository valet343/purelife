...
class SeoPro {
    ...
    public function prepareRoute($parts) {
        ...
        if (!empty($parts) && is_array($parts)) {
            ...
            if(!$query) {
				...
				// It is DIFFERENT with seo_url.php modification (!)
				$new_url = false;
				...
				if ($query->rows) {
					foreach ($query->rows as $item) {
						...
						$essence_to_keys[mb_strtolower($item['seo_url_old'])] = $item['query'];
    ...
    private function detectLanguage() {
        ...
        if (isset($this->request->get['_route_'])) {
            ...
            //fix_dc_pro
            if ($this->config->get('config_seopro_addslash') && substr($this->request->get['_route_'], -1)=='/') {
