...
class ModelExtensionModuleSpeedyMenu extends Model {
	...
	public function MegaMenuTypeFreeLink($data){
		...
		if($data['image']){
			$thumb_menu = $this->model_tool_image->resize($data['image'], 18, 18);
		} else {
			$thumb_menu = $this->model_tool_image->resize('no_image.png', 18, 18);
		}
		if($data['image_hover']){
			$thumb_menu_hover = $this->model_tool_image->resize($data['image_hover'], 18, 18);
		} else {
			$thumb_menu_hover = "";
		}
	...
	public function MegaMenuTypeLink($data){
		...
		if($data['image']){
			$thumb = $this->model_tool_image->resize($data['image'], 18, 18);
		} else {
			$thumb = $this->model_tool_image->resize('no_image.png', 18, 18);
		}
		if($data['image_hover']){
			$thumb_hover = $this->model_tool_image->resize($data['image_hover'], 18, 18);
		} else {
			$thumb_hover = "";
		}
		if(!empty($data['namemenu'])){
			...
			foreach($manufacturers_setting['manufacturers_list'] as $manufacturer_id){
				...
				if($manufacturer){
					...
					if (is_file(DIR_IMAGE . $manufacturer['image'])) {
						$thumb_mr = $this->model_tool_image->resize($manufacturer['image'], 18, 18);
					} else {
						$thumb_mr = $this->model_tool_image->resize('no_image.png', 18, 18);			
					}
