...
class ControllerExtensionModuleArtAqaProduct extends Controller { 
	...
	public function getForm($arg = '') {
		if ($this->config->get('module_art_aqa_product_status') == 1) {
			...
			if (isset($arg['product_name']) && !empty($arg['product_name'])) {
		    	$data['product_name'] = $arg['product_name'];
		    }
			...
			$data['product_question'] = $this->getQuestion($this->request->get['product_id']);
