...
$(document).ready(function() {
	...
	$('#button-menu').on('click', function(e) {
		...
		$('.navbar-dc-pro').removeClass('active');
	});

	// dc_pro

	$('#navbar-dc-pro-menu').on('click', function(e) {
		e.preventDefault();
		$('#column-left').removeClass('active');
		if($(".navbar-dc-pro").hasClass("active")) {
			$('.navbar-dc-pro').removeClass('active');
		} else {
			$('.navbar-dc-pro').addClass('active');
		}
	});

	if($(window).width() <= 768) {
		$("#header #oc-search-div").before($(".change_theme"));
	}
